...
func TestSearchAddressByCEP(t *testing.T) {
	tests := []struct {
		name     string
		cep      string
		expected schemas.Address
	}{
		{"Test traditional CEP", "02010010", schemas.Address{
			CEP:          "02010010",
			Street:       "Street A",
			Neighborhood: "Neighborhood A",
			City:         "City A",
		...
		{"Test CEP with last digits reset for zero", "02010754", schemas.Address{
			CEP:          "02010754",
			Street:       "Street B",
			Neighborhood: "Neighborhood B",
			City:         "City B",
		...
		{"Test CEP not found", "00000000", schemas.Address{}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			address, err := SearchAddressByCEP(tt.cep, mockCEPs)
			if err != nil && tt.expected != (schemas.Address{}) {
